...
//
//  - In retrospect, using 3D with pseudo-2D graphics (what Enter the Gungeon does) would have been much easier and also looked much better.
...
public class Floor : MonoBehaviour
{
    ...
    public GameObject[] BossRooms;     //All boss rooms.
    public GameObject[] ItemRooms;     //All item rooms.
    ...
    public Dictionary<Vector2, Vector2> Orientations = new Dictionary<Vector2, Vector2>();     //A list of Vector2 directions and their opposites.
    ...
    void AddRoom(GameObject LastRoom, Door LastDoor, int Distance, int ChosenDistance, bool ForceEnd = false)
    {
        ...
        if (LastRoom.TryGetComponent<Room>(out var roomScr))
        {
            ...
            Vector3 NextRoomPos;
            int ChosenDist = UnityEngine.Random.Range(MinRoomDistance, MaxRoomDistance);
            ...
            do
            {
                ...
                else if (!ForceEnd && isEnd && Distance < MinPathDistance)
                {
                    found = false;
            ...
            if (CurrentRoom.TryGetComponent<Room>(out var nextRoomScr))
            {
                ...
                {
                    Distance++;
                    bool EndOfTheLine = (Distance >= ChosenDistance || Rooms.Count >= MaxRooms);
                    ...
                    if (door.RoomDir != LastDoor.RoomDir * -1) //This prevents us from messing with the door we just came from.
                    {
                        ...
                        else
                        {
                            ...
                            {
                                AddRoom(CurrentRoom, door, Distance, ChosenDistance, Distance == ChosenDistance - 1 || Rooms.Count == MaxRooms - 1);
    ...
    public void MakeMap()
    {
        ...
        if (CurrentRoom.TryGetComponent<Room>(out var roomScr))
        {
            ...
            foreach (Door door in RandomizeList(roomScr.Doors))
            {
                ...
                else
                {
                    ...
                    {
                        AddRoom(CurrentRoom, door, 1, UnityEngine.Random.Range(MinPathDistance, MaxPathDistance));
                        Debug.DrawRay(door.transform.position, door.RoomDir * ChosenDist, Color.green, 999);
                    }
                }
            }
        ...
        ReplaceLastWithBossRoom();
        ReplaceRandomWithItemRoom();
    }

    void ReplaceLastWithBossRoom()
    {
        GameObject LastRoom = Rooms[Rooms.Count - 1];

        List<GameObject> RoomList = BossRooms.ToList<GameObject>();
        GameObject NextRoom;
        bool found;
        List<Vector2> Required = LastRoom.GetComponent<Room>().DoorDirections;
        List<Vector2> Forbidden = new List<Vector2>();
        ...
        if (!found)
        {
            Debug.Log("Failed to find a valid boss room.");
        }
        else
        {
            GameObject NewRoom = Instantiate(NextRoom, LastRoom.transform.position, Quaternion.identity);

            if (NewRoom.TryGetComponent<Room>(out Room room))
            {
                foreach (Door door in room.Doors)
                {
                    if (!LastRoom.GetComponent<Room>().DoorDirections.Contains(door.RoomDir))
                    {
                        door.MakeDormant();
                    }
                }
    ...
    void ReplaceRandomWithItemRoom()
    {
        GameObject LastRoom = Rooms[UnityEngine.Random.Range(1, Rooms.Count - 1)];

        List<GameObject> RoomList = ItemRooms.ToList<GameObject>();
        GameObject NextRoom;
        bool found;
        List<Vector2> Required = LastRoom.GetComponent<Room>().DoorDirections;
        List<Vector2> Forbidden = new List<Vector2>();
        ...
        if (!found)
        {
            Debug.Log("Failed to find a valid item room.");
        }
        else
        {
            GameObject NewRoom = Instantiate(NextRoom, LastRoom.transform.position, Quaternion.identity);
