...
public class EnemyWithGun : Enemy
{
    ...
    public override void PlaceInRoom()
    {
        base.PlaceInRoom();
    ...
    public virtual void DetectShoot()
    {
        ...
        if (CurrentWeapon.AmountLoaded < 1)
        {
            CurrentWeapon.ReloadWeapon(ammoBag.GetComponent<AmmoHolder>());
        }
        ...
        if (CurrentWeapon.RemainingPrimaryAttackCooldown <= Time.time && Vector3.Distance(transform.position, Target.transform.position) <= MaxShootDistance && CurrentWeapon.AmountLoaded > 0)
        {
            CurrentWeapon.Shoot(false, ammoBag.GetComponent<AmmoHolder>(), CurrentWeapon.BarrelLocation, Target.transform.position);
        }
        else if (CurrentWeapon.AmountLoaded < CurrentWeapon.ClipSize)
        {
            CurrentWeapon.ReloadWeapon(ammoBag.GetComponent<AmmoHolder>());
        }
    }
