...
//TODO: If a projectile is too fast it can skip over enemies and do nothing. This can be easily fixed with a raycast.
...
public class Projectile : MonoBehaviour
{
    ...
    public AudioClip[] ImpactSounds;        //The sound(s) played when this projectile hits something that isn't a target.
    public AudioClip[] ReloadSounds;    //The sounds to play when this ammo is loaded into a weapon.
    ...
    public float ImpactVolume = 1; //The volume of the sound(s) played when this projectile hits something that isn't a target.
    ...
    public float ClipWeight = 1;  //Space this ammo type takes up in weapon clips

    public float ScreenShakeForceMultiplier = 1;
    public float ScreenShakeDecayMultiplier = 1;
    public float ScreenShakeMaxMultiplier = 1;
    ...
    {
        Projectile_TriggerEntered(collision);
    }

    public virtual void Projectile_TriggerEntered(Collider2D collision)
    {
        bool ValidCollision = true;
        bool HitATarget = false;

        if (collision.gameObject.TryGetComponent<Projectile>(out Projectile otherBullet))
        {
            ValidCollision = otherBullet.Owner.GetComponent<LivingEntity>().Team != Owner.GetComponent<LivingEntity>().Team;
        }
        else if (collision.isTrigger)
        {
            ValidCollision = false;
        }
        else if (collision.gameObject.TryGetComponent<LivingEntity>(out LivingEntity other))
        {
            if (other.Team != Team)
            {
        ...
        if (ValidCollision)
        {
            Projectile_Collided(!HitATarget);
        }
    }

    public virtual void Projectile_Collided(bool PlayImpact)
    {
        if (ImpactParticle != null)
        {
            GameObject part = Instantiate(ImpactParticle, transform.position, Quaternion.identity);
            part.GetComponent<ParticleSystem>().Play();
        }
        if (PlayImpact)
        {
            AudioSource.PlayClipAtPoint(ImpactSounds[Random.Range(0, ImpactSounds.Length)], transform.position, ImpactVolume);
        }
